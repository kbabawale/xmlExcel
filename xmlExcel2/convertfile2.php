...
try{
    ...
    foreach($feed->message->documentCommand->documentCommandOperand->debitCreditAdvice as $item){
        ...
        $ClaimArray['INVOICENUMBER_'.$Counter] = $item->debitCreditDetail[0]->debitCreditReference[0]->reference->entityIdentification->uniqueCreatorIdentification;
        $ClaimArray['GRVNUMBER_'.$Counter] = ''; 
        $ClaimArray['TRUCKDETAILS1_'.$Counter] = ''; 
        
        if(isset($item->debitCreditDetail[0]->debitCreditReference[3]->reference->entityIdentification->uniqueCreatorIdentification)){
            $ClaimArray['TRUCKDETAILS1_'.$Counter] = $item->debitCreditDetail[0]->debitCreditReference[3]->reference->entityIdentification->contentOwner->additionalPartyIdentification->additionalPartyIdentificationValue;
            $ClaimArray['GRVNUMBER_'.$Counter] = $item->debitCreditDetail[0]->debitCreditReference[2]->reference->entityIdentification->uniqueCreatorIdentification;

        }else{
            $ClaimArray['TRUCKDETAILS1_'.$Counter] = $item->debitCreditDetail[0]->debitCreditReference[2]->reference->entityIdentification->contentOwner->additionalPartyIdentification->additionalPartyIdentificationValue;
            $ClaimArray['GRVNUMBER_'.$Counter] = $item->debitCreditDetail[0]->debitCreditReference[1]->reference->entityIdentification->uniqueCreatorIdentification;
        ...
        if(preg_match('/-/', $ClaimArray['TRUCKDETAILS1_'.$Counter])){
            //get first occurrence of hyphen.
            $pos = strpos($ClaimArray['TRUCKDETAILS1_'.$Counter], '-');
            //extract string after hyphen
            $beforehyphen = substr($ClaimArray['TRUCKDETAILS1_'.$Counter], 0, $pos);
            $afterhyphen = substr($ClaimArray['TRUCKDETAILS1_'.$Counter], $pos);

            $ClaimArray['DRIVERNAME_'.$Counter] = $beforehyphen;
            $ClaimArray['VEHICLENUMBER_'.$Counter] = $afterhyphen;
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('D'."$rownum", 'Return GRN No/Credit Note/CCV Number');
    ...
    { 
        ...
        { 
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('D'."$rownum", $ClaimArray['GRVNUMBER_'.$i]);
            $objPHPExcel->getActiveSheet()->SetCellValue('E'."$rownum", $ClaimArray['INVOICENUMBER_'.$i]);
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('L'."$rownum", 'OVERCHARGE');
